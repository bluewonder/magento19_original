...
class Wishlattedesk_Customerlistreport_Block_Adminhtml_Report_Customer_Render_Secondrls extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{
    public function render(Varien_Object $row)
    {
        ...
        if (!empty($relationshipData[$row->getId()]) && !empty($relationshipData[$row->getId()][1])) {
            $html = '<span>'.$relationshipData[$row->getId()][1]['relationship'].'</span>';
        }
        return $html;
    }
}