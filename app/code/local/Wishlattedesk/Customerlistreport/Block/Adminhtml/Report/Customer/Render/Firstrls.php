...
class Wishlattedesk_Customerlistreport_Block_Adminhtml_Report_Customer_Render_Firstrls extends Mage_Adminhtml_Block_Widget_Grid_Column_Renderer_Abstract
{
    static  $i = 0;
    public function render(Varien_Object $row)
    {
        ...
        if (!empty($relationshipData[$row->getId()]) && !empty($relationshipData[$row->getId()][0])) {
            $html = '<span>'.$relationshipData[$row->getId()][0]['relationship'].'</span>';
        }
        self::$i++;
        return $html;
    }
